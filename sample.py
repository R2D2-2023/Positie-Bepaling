...
while lidar.open():
    ...
    if( map_perspective == "N"):
        img_map = cv2.imread('./Map_Noord.png', cv2.IMREAD_GRAYSCALE)
        img_map = cv2.rotate(img_map, cv2.ROTATE_90_CLOCKWISE)
    elif( map_perspective == "E"):
        img_map = cv2.imread('./Map_Oost.png', cv2.IMREAD_GRAYSCALE)
        print(img_map.shape)
        img_map = cv2.resize(img_map, (100,img_map.shape[0]))
    elif( map_perspective == "S"):
        img_map = cv2.imread('./Map_Zuid3.png', cv2.IMREAD_GRAYSCALE)
        img_map = cv2.rotate(img_map, cv2.ROTATE_90_CLOCKWISE)
